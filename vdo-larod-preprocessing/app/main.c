...
int main(int argc, char** argv) {
    ...
    if (!parseChipArg(argv[1], &infChip)) {
        fprintf(stderr, "Could not parse INF_CHIP argument %s: %s\n", argv[1],
                strerror(errno));

        return ret;
    }

    printf("Connecting to larod\n");

    if (!larodConnect(&conn, &larodError)) {
        fprintf(stderr, "Could not connect to larod (%d): %s\n",
                larodError->code, larodError->msg);

        return ret;
    }

    printf("Setting up inference model\n");
    ...
    printf("Setting up preprocessing model\n");

    if (!setupPreprocModel(conn, infChip, &preprocModel)) {
        goto end;
    }

    printf("Setting up and starting VDO stream\n");

    if (!setupVdoStream(&vdoStream)) {
        goto end;
    }

    printf("Starting to run larod jobs\n");
    ...
    for (size_t i = 0; i < NUM_ROUNDS; i++) {
        ...
        if (!larodRunJob(conn, preprocJobReq, &larodError)) {
            fprintf(stderr, "Could not run preprocessing job (%d): %s\n",
                    larodError->code, larodError->msg);
        ...
        if (!larodRunJob(conn, infJobReq, &larodError)) {
            fprintf(stderr, "Could not run inference job (%d): %s\n",
                    larodError->code, larodError->msg);
        ...
        if (!vdo_stream_buffer_unref(vdoStream, &vdoBuffer, &vdoError)) {
            fprintf(stderr, "Could not unref buffer of VDO stream: %s\n",
                    vdoError->message);
...
bool setupInferenceModel(larodConnection* const conn, const larodChip chip,
    ...
    if (fd < 0) {
        fprintf(stderr, "Could not open inference model file %s: %s\n",
    ...
    *model = larodLoadModel(conn, fd, chip, LAROD_ACCESS_PRIVATE, NULL, NULL,
                            &larodError);
    if (!*model) {
        fprintf(stderr, "Could not load inference model (%d): %s\n",
                larodError->code, larodError->msg);
...
bool setupPreprocModel(larodConnection* const conn, const larodChip infChip,
    ...
    if (!modelParams) {
        fprintf(stderr, "Could not create preprocessing model map (%d): %s\n",
                larodError->code, larodError->msg);
    ...
    if (!larodMapSetStr(modelParams, "image.input.format", "nv12",
                        &larodError)) {
        fprintf(stderr, "Could not set preprocessing input format (%d): %s\n",
                larodError->code, larodError->msg);
    ...
    if (!larodMapSetIntArr2(modelParams, "image.input.size", VDO_OUTPUT_WIDTH,
        ...
        fprintf(stderr, "Could not set preprocessing input size (%d): %s\n",
                larodError->code, larodError->msg);
    ...
    if (!larodMapSetStr(modelParams, "image.output.format", preprocOutputFormat,
                        &larodError)) {
        fprintf(stderr, "Could not set preprocessing output format (%d): %s\n",
                larodError->code, larodError->msg);
    ...
    if (!larodMapSetIntArr2(modelParams, "image.output.size",
                            ...
                            &larodError)) {
        fprintf(stderr, "Could not set preprocessing output size (%d): %s\n",
                larodError->code, larodError->msg);
    ...
    *model = larodLoadModel(conn, -1, preprocChip, LAROD_ACCESS_PRIVATE, NULL,
    ...
    if (!*model) {
        fprintf(stderr, "Could not load preprocessing model (%d): %s\n",
                larodError->code, larodError->msg);
...
bool setupInfJobRequest(larodConnection* const conn,
    ...
    *infInputs =
    ...
    if (!*infInputs) {
        fprintf(stderr, "Could not allocate inference input tensors (%d): %s\n",
                larodError->code, larodError->msg);
    ...
    *infJobReq = larodCreateJobRequest(infModel, *infInputs, 1, *infOutputs, 1,
                                       NULL, &larodError);
    if (!*infJobReq) {
        fprintf(stderr, "Could not create inference job request (%d): %s\n",
                larodError->code, larodError->msg);
...
bool setupInfOutputMapping(larodTensor* const infOutputTensor,
    ...
    *infOutputFd = larodGetTensorFd(infOutputTensor, &larodError);
    if (*infOutputFd == LAROD_INVALID_FD) {
        fprintf(stderr, "Could not get inference output tensor fd (%d): %s\n",
                larodError->code, larodError->msg);
    ...
    *infOutputBuf =
    ...
    if (*infOutputBuf == MAP_FAILED) {
        fprintf(stderr, "Could not map inference output tensor's fd : %s\n",
                strerror(errno));
        *infOutputBuf = NULL;
...
bool setupVdoStream(VdoStream** const vdoStream) {
    ...
    if (!map) {
        fprintf(stderr, "Could not create VDO map\n");
    ...
    *vdoStream = vdo_stream_new(map, larodVdoUtilsDestroyTensor, &vdoError);
    if (!*vdoStream) {
        fprintf(stderr, "Could not create VDO stream: %s\n",
    ...
    if (!vdo_stream_start(*vdoStream, &vdoError)) {
        fprintf(stderr, "Could not start VDO stream: %s\n", vdoError->message);
...
bool setupPreprocJobRequest(larodConnection* const conn,
    ...
    *vdoBuffer = vdo_stream_get_buffer(vdoStream, &vdoError);
    if (!*vdoBuffer) {
        fprintf(stderr, "Could not get buffer from VDO stream: %s\n",
                vdoError->message);
        g_clear_error(&vdoError);
...
bool displayInfOutput(const uint8_t* const infOutputBuf,
    ...
    if (dataType == LAROD_TENSOR_DATA_TYPE_UINT8) {
        ...
        for (size_t i = 0; i < NUM_TOP_SCORES; i++) {
            printf("%zu. idx %zu labeled \"%s\" with score %.2f\n", i + 1,
        ...
        fprintf(stderr, "Only models with either UINT8 or FLOAT32 output types "
                        "are supported\n");
